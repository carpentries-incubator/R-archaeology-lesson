...
To create a new column of diameter in kg:

```{r, purl = FALSE}
surveys %>%
  mutate(diameter_kg = diameter / 1000)
```
...
`NA` values because some animals had escaped before their diagnostic and body diameters
...
It is sometimes useful to rearrange the result of a query to inspect the values. For instance, we can sort on `min_diameter` to put the lighter ceramic_type first:
...
the *albigula* ceramic_type that are not specified for its diagnostic (i.e. `NA`).
...
## hindfoot length for each ceramic_type (using `period`). Also add the number of
## observations (hint: see `?n`).
...
with each record (the unit), values such as the diameter or diagnostic of each animal
associated with each record. What if instead of comparing records, we
wanted to compare the different mean diameter of each decoration_type between plots? (Ignoring `plot_type` for simplicity).
...
relationship between the diameter of different genera within, and between, the
...
We could now plot comparisons between the diameter of genera (one is called a decoration_type, multiple are called genera) in different plots,
...
>   summarize(n_genera = n_distinct(decoration_type)) %>%
...
## 4. With this new data set, calculate the average of each `measurement` in each `year` for each different `plot_type`. Then `spread()` them into a data set with a column for `length` and `diameter`. *Hint*: You only need to specify the key and value columns for `spread()`.
```
...
Let's start by removing observations of animals for which `diameter` and `length` are missing, or the `diagnostic` has not been determined:
...
Because we are interested in plotting how ceramic_type abundances have changed
...
we are going to create a data set that counts how often each ceramic_type has been
observed, and filter out the rare ceramic_type; then, we will extract only the
observations for these more common ceramic_type:
...
##  Now remove rare ceramic_type in two steps. First, make a list of ceramic_type which
