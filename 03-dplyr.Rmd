...
```{r, results = 'hide', purl = FALSE}
select(surveys, plot_id, period, weight)
```
...
surveys_sml <- select(surveys2, period, sex, weight)
```
...
```{r, purl = FALSE}
surveys_sml <- select(filter(surveys, weight < 5), period, sex, weight)
```
...
to keep only the `period`, `sex`, and `weight` columns. Since `%>%` takes
...
for rows with `weight < 5`, *then* we `select`ed columns `period`, `sex`,
...
> length for each species (using `period`). Also add the number of
...
> `decoration_type`, `period`, and `weight`.
>
> ```{r, answer=TRUE, purl=FALSE}
> surveys %>%
>     filter(!is.na(weight)) %>%
...
>     select(year, decoration_type, species, weight) %>%
>     arrange(year)
> ```
...
## hindfoot length for each species (using `period`). Also add the number of
## observations (hint: see `?n`).

##  3. What was the heaviest animal measured in each year? Return the
##  columns `year`, `decoration_type`, `period`, and `weight`.
```
...
wanted to compare the different mean weight of each decoration_type between plots? (Ignoring `plot_type` for simplicity).
...
in `decoration_type` would become the names of column variables and the cells would contain the values of the mean weight observed on each plot.
...
average decoration_type weight per plot instead of recordings per date.
...
Using `spread()` to key on `decoration_type` with values from `mean_weight` this becomes
24 observations of 11 variables, one row for each plot.

```{r, purl=FALSE}
surveys_spread <- surveys_gw %>%
  spread(key = decoration_type, value = mean_weight)
...
We could now plot comparisons between the weight of genera (one is called a decoration_type, multiple are called genera) in different plots,
although we may wish to fill in the missing values first.

```{r, purl=FALSE}
surveys_gw %>%
  spread(decoration_type, mean_weight, fill = 0) %>%
  head()
```
...
`decoration_type` and value called `mean_weight` and use all columns except `plot_id` for
the key variable. Here we exclude `plot_id` from being `gather()`ed.

```{r, purl=FALSE}
surveys_gather <- surveys_spread %>%
  gather(key = "decoration_type", value = "mean_weight", -plot_id)
...
```{r, purl=FALSE}
surveys_spread %>%
  gather(key = "decoration_type", value = "mean_weight", Baiomys:Spermophilus) %>%
  head()
```
...
surveys_complete <- surveys_complete %>%
  filter(period %in% species_counts$period)
```

```{r, eval=FALSE, purl=TRUE, echo=FALSE}
### Create the dataset for exporting:
##  Start by removing observations for which the `period`, `weight`,
