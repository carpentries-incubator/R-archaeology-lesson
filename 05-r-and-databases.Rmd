...
Note that the columns `plot_type`, `manufacture_technique`, `decoration_type`, and `species` are missing.
...
than 5 and keep only the period, sex, and weight columns.
...
`period`, `sex` and `weight` columns. Great!
...
Just like the first `select(period, sex, weight)` call, the `select(-sex)`
...
`period` or  minimum `weight`. That's why R doesn't retrieve the full set
...
>  The query should return counts of decoration_type by plot type.
>
...
## decoration_type caught in the different plot types.
...
belonging to each decoration_type found in each plot type. But what if we were interested
...
unique_genera <- left_join(surveys, plots) %>%
    ...
    group_by(plot_type) %>%
    summarize(
        n_genera = n_distinct(decoration_type)
    ) %>%
    collect()
```
