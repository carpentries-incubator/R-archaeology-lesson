...
source("setup.R")
surveys_complete <- read_csv(file = "data_raw/ceramics_data.csv", col_types = cols()) %>%
  drop_na(diameter, length, diagnostic) %>%
  group_by(period) %>%
  filter(n() >= 50) %>%
  ungroup()
```
...
```{r, eval = FALSE, purl = FALSE}
ggplot(data = surveys_complete, mapping = aes(x = diameter, y = length))
```
...
ggplot(data = surveys_complete, aes(x = diameter, y = length)) +
  geom_point()
```
...
surveys_plot <- ggplot(data = surveys_complete,
                       mapping = aes(x = diameter, y = length))
...
Or to color each species in the plot differently, you could use a vector as an input to the argument **color**. **`ggplot2`** will provide a different color corresponding to different values in the vector. Here is an example where we color with **`period`**:


```{r color-by-species-1, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = diameter, y = length)) +
    geom_point(alpha = 0.1, aes(color = period))
```

> ### Challenge
>
> Use what you just learned to create a scatter plot of `diameter` over
> `period` with the plot types showing in different colors.
...
We can use boxplots to visualize the distribution of diameter within each species:
...
>
> - Represent diameter on the log~10~ scale; see `scale_y_log10()`.
>
> So far, we've looked at the distribution of diameter within species. Try making
> a new plot to explore the distribution of another variable within each species.
>
> - Create boxplot for `length`.  Overlay the boxplot layer on a jitter
...
Let's calculate number of counts per year for each decoration_type. First we need
to group the data and count records within each group:

```{r, purl=FALSE}
yearly_counts <- surveys_complete %>%
  count(year, decoration_type)
```
...
together. We need to tell ggplot to draw a line for each decoration_type by modifying
the aesthetic function to include `group = decoration_type`:

```{r time-series-by-species, purl = FALSE}
ggplot(data = yearly_counts, aes(x = year, y = n, group = decoration_type)) +
    geom_line()
```
...
ggplot(data = yearly_counts, aes(x = year, y = n, color = decoration_type)) +
    geom_line()
```
...
yearly_counts %>%
    ggplot(mapping = aes(x = year, y = n, color = decoration_type)) +
    geom_line()
```
...
use it to make a time series plot for each decoration_type:

```{r first-facet, purl = FALSE}
ggplot(data = yearly_counts, aes(x = year, y = n)) +
    geom_line() +
    facet_wrap(facets = vars(decoration_type))
```

Now we would like to split the line in each plot by the diagnostic of each individual
measured. To do that we need to make counts in the data frame grouped by `year`,
`decoration_type`, and `diagnostic`:

```{r, purl = FALSE}
 yearly_diagnstic_counts <- surveys_complete %>%
                      count(year, decoration_type, diagnostic)
```

We can now make the faceted plot by splitting further by diagnostic using `color`
(within a single plot):

```{r facet-by-decoration_type-and-diagnostic, purl=FALSE}
ggplot(data = yearly_diagnostic_counts, mapping = aes(x = year, y = n, color = diagnostic)) +
  geom_line() +
  facet_wrap(facets =  vars(decoration_type))
```

We can also facet both by diagnostic and decoration_type:

```{r average-diameter-time-facet-both, purl=FALSE, fig.width=9.5}
ggplot(data = yearly_diagnostic_counts,
       mapping = aes(x = year, y = n, color = diagnostic)) +
  geom_line() +
  facet_grid(rows = vars(diagnostic), cols =  vars(decoration_type))
```

You can also organise the panels only by rows (or only by columns):

```{r average-diameter-time-facet-diagnostic-rows, purl=FALSE, fig.height=8.5, fig.width=8}
# One column, facet by rows
ggplot(data = yearly_diagnostic_counts,
       mapping = aes(x = year, y = n, color = diagnostic)) +
  geom_line() +
  facet_grid(rows = vars(decoration_type))
```

```{r average-diameter-time-facet-diagnostic-columns, purl=FALSE, fig.width=9.5, fig.height=5}
# One row, facet by column
ggplot(data = yearly_diagnostic_counts,
       mapping = aes(x = year, y = n, color = diagnostic)) +
  geom_line() +
  facet_grid(cols = vars(decoration_type))
```
...
```{r facet-by-species-and-diagnostic-white-bg, purl=FALSE}
 ggplot(data = yearly_diagnostic_counts,
        mapping = aes(x = year, y = n, color = diagnostic)) +
     geom_line() +
     facet_wrap(vars(decoration_type)) +
     theme_bw()
```
...
> ### Challenge
>
> Use what you just learned to create a plot that depicts how the average diameter
> of each species changes through the years.
>
> ```{r average-diameter-time-series, answer=TRUE, purl=FALSE}
> yearly_diameter <- surveys_complete %>%
>                 group_by(year, period) %>%
>                  summarize(avg_diameter = mean(diameter))
> ggplot(data = yearly_diameter, mapping = aes(x=year, y=avg_diameter)) +
>    geom_line() +
...
##  average diameter of each species changes through the years.
...
ggplot(data = yearly_diagnostic_counts, aes(x = year, y = n, color = diagnostic)) +
    geom_line() +
    facet_wrap(vars(decoration_type)) +
...
labels. We can also modify the facet label text (`strip.text`) to italicize the decoration_type
names:
...
ggplot(surveys_complete, aes(x = period, y = length)) +
    geom_boxplot() +
    grey_theme
```
...
plot_diameter <- ggplot(data = surveys_complete, aes(x = period, y = diameter)) +
  geom_boxplot() +
  labs(x = "Species", y = expression(log[10](diameter))) +
  scale_y_log10()

plot_count <- ggplot(data = yearly_counts, aes(x = year, y = n, color = decoration_type)) +
  geom_line() +
  labs(x = "Year", y = "Abundance")

plot_diameter / plot_count + plot_layout(heights = c(3, 2))
```
...
my_plot <- ggplot(data = yearly_diagnostic_counts,
                  aes(x = year, y = n, color = diagnostic)) +
    geom_line() +
    facet_wrap(vars(decoration_type)) +
    labs(title = "Observed genera through time",
        x = "Year of observation",
...
plot_combined <- plot_diameter / plot_count + plot_layout(heights = c(3, 2))
