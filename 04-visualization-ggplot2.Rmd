...
ggplot(data = surveys_complete, mapping = aes(x = period, y = weight)) +
    geom_boxplot()
```
...
`decoration_type`, and `sex`:

```{r, purl = FALSE}
 yearly_sex_counts <- surveys_complete %>%
                      count(year, decoration_type, sex)
```

We can now make the faceted plot by splitting further by sex using `color`
(within a single plot):

```{r facet-by-decoration_type-and-sex, purl=FALSE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(facets =  vars(decoration_type))
```

We can also facet both by sex and decoration_type:

```{r average-weight-time-facet-both, purl=FALSE, fig.width=9.5}
ggplot(data = yearly_sex_counts,
       mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_grid(rows = vars(sex), cols =  vars(decoration_type))
```
...
ggplot(surveys_complete, aes(x = period, y = hindfoot_length)) +
    geom_boxplot() +
    grey_theme
```
...
plot_weight <- ggplot(data = surveys_complete, aes(x = period, y = weight)) +
  geom_boxplot() +
